...
const startSock = async (num, sessionDir, message) => {
  ...
  async function connect() {
    ...
    let pairingCode = "";
    if ((!sock.authState.creds.me || !sock.authState.creds.me.id) && !restarted) {
    ...
    sock.ev.process(async (events) => {
      if (events["connection.update"]) {
        ...
        const status = (lastDisconnect && lastDisconnect.error && lastDisconnect.error.output && lastDisconnect.error.output.statusCode) ? lastDisconnect.error.output.statusCode : undefined;
        if (
          connection === "close" &&
